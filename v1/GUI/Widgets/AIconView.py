from PyAsoka.GUI.Widgets.AWidget import AWidget, QPaintEvent, Styles, QPainter, QColor, QRect, QPoint, QPen, Qt

from PySide6.QtWidgets import QLabel
from PySide6.QtGui import QPixmap, QResizeEvent, QPainterPath


class AIconView(AWidget):
    def __init__(self, image, frame: int = False, style=None, **kwargs):
        if style is None:
            style = Styles.frameWidget()
        AWidget.__init__(self, style=style, **kwargs)
        self.image = QPixmap()
    ...
    def paintEvent(self, event: QPaintEvent):
        if self.__is_content_visible__():
            ...
            if self.is_frame:
                painter.setPen(QPen(self.colors.frame, indent * 2))
    ...
    def resizeEvent(self, event: QResizeEvent):
        self.image = self.image.scaled(event.size().width(), event.size().height(), Qt.IgnoreAspectRatio, Qt.SmoothTransformation)
