from PyAsoka.GUI.Widgets.AWidget import AWidget, QPaintEvent, QPoint, QSize, QRect, QPen, QBrush
from PyAsoka.GUI.Widgets.AMatrixMenu.AMatrixMenuScene import AMatrixMenuScene, AMatrixMenuUnit, AMatrixMenuButton
from PyAsoka.src.Instruments.Menu import Menu
from PyAsoka.GUI.Styles import Styles
...
class AMatrixMenu(AWidget):
    ...
    def key_pressed(self, key):
        kb = API.Keyboard
        if key == kb.Symbol('w') and not self.scene.current_horizontal:
            self.up()
        if key == kb.Symbol('s'):
            if not self.scene.current_horizontal:
                ...
                self.enter()
        if key == kb.Symbol('a') and self.scene.current_horizontal:
            self.left()
        if key == kb.Symbol('d'):
            if self.scene.current_horizontal:
                ...
                self.enter()
        if key == kb.Key.enter or key == kb.Symbol('f'):
            self.enter()
        if key == kb.Symbol('e'):
            self.close()
        if key == kb.Symbol('q'):
