...
from PyAsoka.Graphics.ImageArray import ImageArray
...
class Image(ImageArray):
    ...
    def find_contour_linear(self, point1: Point, point2: Point, thickness, smoothing, sensitivity,
        ...
        from PyAsoka.Maths.Geometry.Straight import Straight
        from PyAsoka.Maths.Geometry.Geometry import Geometry
        ...
        if approximation:
            ...
            OX_vect = AVector(APoint(0, 0), APoint(1, 0))
            angles = []
            for i in range(len(sections_avg) - 1):
                vector = AVector(sections_avg[i].toAPoint(), sections_avg[i + 1].toAPoint())
