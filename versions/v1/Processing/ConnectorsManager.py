    ...
    def remove(self, value):
        if isinstance(value, AConnector):
            try:
                ...
                Log.warning(f"Ошибка удаления коннектора (id = {value.id}) из менеджера. Коннектор не найден.")
...
class ConnectorsProcessManager(ConnectorsManager):
    def add(self, connector: AConnector):
        ...
        AProcess.current_process.core.channel.send(ProcessMessage(Headers.CONNECTOR_ADDED, (connector.id.id, connector.id.__unique_name__)))
...
class ConnectorsCoreManager(ConnectorsManager):
    def add(self, connector: AConnector):
        ...
        ACore.current_process.multi_connectors.add(connector.id, ProcessCutaway(ACore.current_process.name, None))
